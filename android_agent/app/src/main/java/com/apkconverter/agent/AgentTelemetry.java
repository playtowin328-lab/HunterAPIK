    ...
    static String toJson(Context context) {
        ...
        try {
            return new JSONObject()
                    ...
                    .put("accessibility_enabled_in_settings", BuildConfig.FULL_CONTROL && TouchControlService.isEnabledInSettings(context))
                    .put("accessibility_state", BuildConfig.FULL_CONTROL ? TouchControlService.getConnectionState(context) : "disabled")
                    .put("accessibility_session_id", TouchControlService.getConnectionSessionId(context))
                    .put("accessibility_session_age", TouchControlService.getConnectionAgeSeconds(context))
                    .put("screen_streaming", BuildConfig.FULL_CONTROL && ScreenCaptureService.isRunning())
                    .put("screen_session_state", ScreenCaptureService.getSessionState(context))
                    .put("screen_session_id", ScreenCaptureService.getSessionId(context))
                    .put("screen_session_age", ScreenCaptureService.getSessionAgeSeconds(context))
                    .put("screen_last_frame_age", ScreenCaptureService.getLastFrameAgeSeconds(context))
                    .put("screen_permission_required", ScreenCaptureService.isPermissionRequired(context))
                    .put("screen_permission_pending", ScreenCaptureService.isPermissionPending(context))
                    .put("screen_stop_reason", ScreenCaptureService.getStopReason(context))
                    .put("screen_width", ScreenCaptureService.getCaptureWidth())
                    .put("screen_height", ScreenCaptureService.getCaptureHeight())
                    .put("screen_rotation", ScreenCaptureService.getCaptureRotation())
                    .put("screen_frame_sequence", ScreenCaptureService.getFrameSequence())
                    .put("screen_display_changes", ScreenCaptureService.getDisplayChanges())
                    ...
                    .put("network_failures", DeviceApiClient.getConsecutiveNetworkFailures())
                    .put("network_backoff_ms", DeviceApiClient.getLastRetryDelayMs())
                    .put("network_error", DeviceApiClient.getLastNetworkError())
                    .put("command_transport", "long_poll")
                    .put("command_wait_seconds", DeviceApiClient.COMMAND_WAIT_SECONDS)
                    .put("command_replays_prevented", prefs.getInt(AgentConfig.KEY_COMMAND_REPLAYS_PREVENTED, 0))
                    .put("command_receipt_cache_size", CommandReceiptStore.size(context))
                    ...
                    .put("gesture_in_flight", TouchControlService.isGestureInFlight())
                    .put("gesture_completed", prefs.getInt(AgentConfig.KEY_GESTURE_COMPLETED_COUNT, 0))
                    .put("gesture_cancelled", prefs.getInt(AgentConfig.KEY_GESTURE_CANCELLED_COUNT, 0))
                    .put("gesture_rejected", prefs.getInt(AgentConfig.KEY_GESTURE_REJECTED_COUNT, 0))
                    .put("gesture_timeouts", prefs.getInt(AgentConfig.KEY_GESTURE_TIMEOUT_COUNT, 0))
