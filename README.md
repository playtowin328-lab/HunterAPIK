...
Пульт управления устройствами теперь включает продуктовый слой надёжности:

- SLO и доступность каждого устройства за 24 часа;
- error budget, MTTR, число и длительность сбоев;
- прогноз риска с приоритетным устройством и следующим действием;
- отраслевые режимы: универсальный, ритейл, логистика, выездные команды и инфраструктура;
- Hardening Autopilot: staged recovery, anti-flap guard и безопасное укрепление всего флота;
- отдельные rate-limit buckets для агентов за общим NAT, адаптивный ETA и защита от повторного выполнения команд.
...
CONTROL_PIN=безопасный_PIN_минимум_4_цифры
```
...
- `GET /api/devices/history` — история заряда, сети, связи и ошибок за 24 часа
